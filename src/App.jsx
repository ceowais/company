...
             <p className="text-gray-600 dark:text-gray-400">7 Tomb, Towlichowki,<br/>Hyderabad, Telangana India  </p>
          </div>
        </div>
...
// --- NEW PAGE: JOBS CONSULTING ---
const JobsConsulting = ({ navigateTo }) => {
  const [rotation, setRotation] = useState({ x: 0, y: 0 });
  const [activeFaq, setActiveFaq] = useState(null);
...
    const { clientX, clientY } = e;
    const rect = e.currentTarget.getBoundingClientRect();
    const x = ((clientY - rect.top) / rect.height - 0.5) * 20; // 20 deg tilt
    const y = ((clientX - rect.left) / rect.width - 0.5) * 20;
    setRotation({ x: -x, y });
  };

  const prepSteps = [
    { title: "Skill Assessment", desc: "Comprehensive evaluation of your current technical and soft skills.", icon: <Search /> },
    { title: "Gap Analysis", desc: "Identifying missing skills required for your dream roles.", icon: <Activity /> },
    { title: "Targeted Upskilling", desc: "Curated learning paths and resources to bridge the gap rapidly.", icon: <TrendingUp /> },
    { title: "Mock Interviews", desc: "Simulated technical and HR rounds with industry experts.", icon: <Target /> }
  ];

  const cvSteps = [
    { step: "01", title: "Format & Structure", desc: "Adopt an ATS-friendly layout with clean typography, standard margins, and clear logical hierarchy." },
    { step: "02", title: "Executive Summary", desc: "Craft a compelling 3-line summary highlighting your top achievements, core stack, and career trajectory." },
    { step: "03", title: "Experience & Metrics", desc: "List experiences in reverse-chronological order. Use the XYZ formula: 'Accomplished [X] as measured by [Y], by doing [Z]'." },
    { step: "04", title: "Skill Optimization", desc: "Extract exact keywords from targeted job descriptions and weave them naturally into a dedicated skills matrix." }
  ];

  const availableJobs = [
    { id: 1, title: "Senior Frontend Engineer", role: "SDE III", desc: "Lead React architectures and mentor junior developers in building scalable web apps.", location: "Remote / USA", date: "Oct 28, 2025", package: "$140k - $160k" },
    { id: 2, title: "Data Scientist", role: "Data Team", desc: "Build predictive models and optimize our data warehousing solutions using Python & AWS.", location: "London, UK", date: "Nov 02, 2025", package: "£80k - £110k" },
    { id: 3, title: "DevOps Specialist", role: "Infrastructure", desc: "Automate CI/CD pipelines and manage Kubernetes clusters for enterprise clients.", location: "Berlin, DE", date: "Nov 05, 2025", package: "€70k - €95k" },
    { id: 4, title: "Product Manager", role: "Product", desc: "Drive product vision, strategy, and execution across agile development teams.", location: "Hybrid / SF", date: "Nov 10, 2025", package: "$120k - $150k" },
    { id: 5, title: "Cybersecurity Analyst", role: "Security", desc: "Monitor threat landscapes, conduct penetration tests, and enforce zero-trust protocols.", location: "Dubai, UAE", date: "Nov 12, 2025", package: "€85k - €120k" },
  ];

  const faqs = [
    { q: "How long does the job placement process take?", a: "Typically, candidates who follow our structured preparation and CV optimization programs secure interviews within 3-4 weeks and land offers within 6-8 weeks." },
    { q: "Do you help with salary negotiation?", a: "Yes. Our consultants provide market data, compensation benchmarks, and direct scripts to help you negotiate up to 20% higher base salaries." },
    { q: "Are your CV templates ATS-friendly?", a: "100%. We test all our CV templates against industry-standard Applicant Tracking Systems to ensure perfect parsing and high keyword match rates." },
    { q: "Do you guarantee a job?", a: "While we cannot legally guarantee a job, we guarantee that you will be presented with top-tier opportunities and fully equipped to clear the interviews." }
  ];

  return (
    <div className="pt-32 pb-20 relative z-10 animate-fade-in overflow-hidden">
      
      {/* 1. Intro & 3D Boy Section */}
...
                <Target size={16} className="text-[#D4AF37] animate-pulse" />
                <span className="text-[#D4AF37] font-bold tracking-wider text-sm uppercase">Elite Placements</span>
              </div>
              <h1 className="text-5xl md:text-7xl font-black mb-6 leading-tight text-gray-900 dark:text-white">
                Accelerate Your <br/>
                <span className="text-gradient-gold">Career Trajectory</span>
              </h1>
              <p className="text-xl text-gray-600 dark:text-gray-400 mb-8 leading-relaxed max-w-2xl mx-auto lg:mx-0 font-medium">
                We bridge the gap between top-tier talent and visionary tech companies. Navigate the hiring landscape with our insider guidance, ATS-beating CVs, and targeted interview preparation.
              </p>
              <button onClick={() => navigateTo('contact')} className="px-8 py-4 bg-[#D4AF37] text-black font-bold rounded-lg hover:bg-[#e0c055] transition-all shadow-[0_10px_30px_rgba(212,175,55,0.4)] flex items-center gap-2 mx-auto lg:mx-0">
                Book Career Session <ChevronRight size={20} />
              </button>
            </Reveal>
          </div>

          <div className="w-full lg:w-1/2 perspective-2000 py-10" onMouseMove={handleMouseMove} onMouseLeave={() => setRotation({ x: 0, y: 0 })}>
            <Reveal delay={200}>
              <div 
                className="relative w-full max-w-md mx-auto h-[450px] md:h-[550px] rounded-[2.5rem] transition-transform duration-300 ease-out preserve-3d group"
                style={{ transform: `rotateX(${rotation.x}deg) rotateY(${rotation.y}deg)` }}
              >
                {/* Glow Backdrop */}
                <div className="absolute inset-0 bg-[#D4AF37]/20 blur-[50px] rounded-full animate-pulse-gold transform translate-z-[-50px]"></div>
                
                {/* Main 3D Image */}
                <img 
                  src="https://images.unsplash.com/photo-1552664730-d307ca884978?q=80&w=800&auto=format&fit=crop" 
                  alt="Candidate" 
                  className="w-full h-full object-cover rounded-[2.5rem] shadow-[0_30px_60px_rgba(0,0,0,0.5)] border-4 border-gray-800 dark:border-white/10 group-hover:border-[#D4AF37]/50 transition-colors" 
                />
                
                {/* Floating 3D CV Card (Right) */}
                <div className="absolute -right-8 md:-right-16 top-20 glass-panel p-6 rounded-2xl border border-[#D4AF37]/50 shadow-[0_30px_60px_rgba(0,0,0,0.4)] w-64 bg-white/95 dark:bg-[#0a0a0a]/95 backdrop-blur-xl animate-float" style={{ transform: 'translateZ(80px)', animationDuration: '6s' }}>
                  <div className="flex items-center gap-4 mb-4">
                    <div className="w-12 h-12 bg-[#D4AF37]/20 rounded-full flex items-center justify-center border border-[#D4AF37]/40 shadow-inner">
                      <FileCheck className="text-[#D4AF37]" size={24} />
                    </div>
                    <div>
                      <h4 className="font-bold text-gray-900 dark:text-white text-sm uppercase tracking-wider">ATS Match</h4>
                      <span className="text-[#D4AF37] font-black text-2xl drop-shadow-[0_0_5px_rgba(212,175,55,0.5)]">98%</span>
                    </div>
                  </div>
...
                       </div>
                       <span className="text-xs font-bold text-green-500">Keywords</span>
                    </div>
                    <div className="flex items-center gap-2">
                       <div className="h-1.5 flex-grow bg-gray-200 dark:bg-gray-800 rounded-full overflow-hidden">
                         <div className="h-full bg-[#D4AF37] w-[90%]"></div>
                       </div>
                       <span className="text-xs font-bold text-[#D4AF37]">Formatting</span>
                    </div>
                  </div>
                </div>

                {/* Floating Badge (Left) */}
                <div className="absolute -left-6 md:-left-10 bottom-24 bg-green-500 text-white font-black px-6 py-3 rounded-xl shadow-[0_15px_30px_rgba(34,197,94,0.4)] animate-float flex items-center gap-2 border border-green-400" style={{ transform: 'translateZ(50px)', animationDuration: '4s', animationDelay: '1s' }}>
                  Offer Accepted <Award size={20} className="animate-bounce" />
                </div>
              </div>
            </Reveal>
          </div>
...
      {/* 2. Job Preparation & Animated Graph */}
...
          <Reveal>
            <div className="text-center mb-20">
              <h2 className="text-3xl md:text-5xl font-extrabold mb-6 text-gray-900 dark:text-white">Preparation <span className="text-[#D4AF37]">Framework</span></h2>
              <p className="text-lg text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">A systematic approach to elevate your employability and confidence metrics over time.</p>
            </div>
          </Reveal>
...
                <Reveal key={idx} delay={idx * 150}>
                  <div className="glass-panel p-6 rounded-2xl group hover:-translate-y-2 hover:shadow-[0_15px_30px_rgba(212,175,55,0.15)] hover:border-[#D4AF37] transition-all duration-300">
                    <div className="w-12 h-12 bg-[#D4AF37]/10 rounded-xl flex items-center justify-center text-[#D4AF37] mb-4 group-hover:scale-110 group-hover:bg-[#D4AF37] group-hover:text-black transition-all">
                      {step.icon}
                    </div>
                    <h3 className="font-bold text-gray-900 dark:text-white text-lg mb-2">{step.title}</h3>
                    <p className="text-gray-600 dark:text-gray-400 text-sm font-medium">{step.desc}</p>
                  </div>
                </Reveal>
              ))}
            </div>
...
              <Reveal delay={300}>
                <div className="glass-panel p-8 rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.1)] dark:shadow-none border border-gray-200 dark:border-white/10 relative overflow-hidden group">
                  <div className="absolute inset-0 bg-gradient-to-tr from-[#D4AF37]/5 to-transparent"></div>
                  <h4 className="text-xl font-bold mb-8 text-gray-900 dark:text-white flex items-center gap-2 relative z-10">
                    <TrendingUp className="text-[#D4AF37]" /> Candidate Confidence Metric
                  </h4>
                  
                  {/* SVG Graph animated via .active .line-path class inside Reveal */}
                  <div className="relative w-full h-48 sm:h-64 flex items-end justify-between px-2 sm:px-6 pb-6 border-b border-l border-gray-300 dark:border-gray-700 z-10">
                    <svg className="absolute inset-0 w-full h-full pb-6 pl-6 overflow-visible z-20" preserveAspectRatio="none">
                       {/* Animated Glow Line */}
                       <path 
                          className="line-path text-[#D4AF37]" 
                          fill="none" 
...
                          strokeLinecap="round"
                          strokeLinejoin="round"
                          d="M 10 200 Q 80 180 150 150 T 300 80 T 500 20" 
...
                    </svg>
                    
                    {/* Animated Bar charts mimicking the line */}
                    {['Week 1', 'Week 2', 'Week 3', 'Week 4'].map((week, i) => (
                      <div key={i} className="flex flex-col items-center gap-4 z-10 relative group-hover:-translate-y-2 transition-transform duration-700 w-12" style={{ transitionDelay: `${i * 100}ms` }}>
                        <div className="w-full bg-[#D4AF37] rounded-t-sm shadow-[0_0_10px_rgba(212,175,55,0.4)] opacity-80" style={{ height: `${(i+1) * 20 + 20}px` }}></div>
                        <span className="absolute -bottom-6 text-xs font-bold text-gray-500 whitespace-nowrap">{week}</span>
                      </div>
                    ))}
                  </div>
                  <div className="mt-8 flex justify-between text-xs font-bold text-gray-500 uppercase tracking-widest relative z-10">
                    <span>Initiation</span>
                    <span className="text-[#D4AF37]">Interview Ready</span>
                  </div>
                </div>
              </Reveal>
            </div>
...
      {/* 3. Detailed CV Making Timeline */}
      <section className="py-24 px-6 max-w-4xl mx-auto relative z-10">
        <Reveal>
          <div className="text-center mb-20">
            <h2 className="text-3xl md:text-5xl font-extrabold mb-6 text-gray-900 dark:text-white">CV Crafting <span className="text-[#D4AF37]">Protocol</span></h2>
            <p className="text-lg text-gray-600 dark:text-gray-400">Reverse-engineering the perfect resume to bypass ATS filters and captivate human recruiters.</p>
          </div>
        </Reveal>

        <div className="relative">
          {/* Vertical Line */}
          <div className="absolute left-8 md:left-1/2 top-0 bottom-0 w-1 bg-gray-200 dark:bg-gray-800 transform md:-translate-x-1/2 z-0 rounded-full">
            <div className="absolute top-0 left-0 w-full h-1/2 bg-gradient-to-b from-[#D4AF37] to-transparent animate-[line-travel_3s_ease-in-out_infinite]"></div>
          </div>
...
                    {/* Content */}
                    <div className={`w-full md:w-[45%] pl-20 md:pl-0 ${isLeft ? 'md:text-right md:pr-12' : 'md:text-left md:pl-12'}`}>
                      <div className="glass-panel p-6 rounded-2xl hover:border-[#D4AF37] hover:shadow-[0_10px_30px_rgba(212,175,55,0.15)] transition-all duration-300 group cursor-default">
                        <span className="text-3xl font-black text-gray-200 dark:text-gray-800 group-hover:text-[#D4AF37]/20 absolute top-4 right-4 transition-colors">{item.step}</span>
                        <h4 className="text-xl font-bold mb-3 text-gray-900 dark:text-white group-hover:text-[#D4AF37] transition-colors">{item.title}</h4>
                        <p className="text-gray-600 dark:text-gray-400 text-sm leading-relaxed">{item.desc}</p>
                      </div>
                    </div>

                    {/* Timeline Node */}
                    <div className="absolute left-8 md:left-1/2 transform -translate-x-1/2 w-8 h-8 rounded-full bg-white dark:bg-[#0a0a0a] border-[3px] border-[#D4AF37] flex items-center justify-center shadow-[0_0_15px_rgba(212,175,55,0.5)] z-20">
...
              <h2 className="text-4xl md:text-5xl font-extrabold mb-6 text-gray-900 dark:text-white">Exclusive <span className="text-gradient-gold">Opportunities</span></h2>
              <p className="text-lg text-gray-600 dark:text-gray-400">Live roles curated strictly for our premium candidate network.</p>
            </div>
          </Reveal>
...
              <div className="hidden lg:grid grid-cols-12 gap-6 px-8 py-4 rounded-2xl bg-[#D4AF37]/10 border border-[#D4AF37]/20 text-[#D4AF37] font-black uppercase tracking-wider text-xs">
                <div className="col-span-3">Job Title & Role</div>
                <div className="col-span-4">Description</div>
                <div className="col-span-2">Location & Date</div>
                <div className="col-span-2">Package</div>
                <div className="col-span-1 text-right">Action</div>
              </div>

              {/* Table Rows (Cards on Mobile, Row on Desktop) */}
              {availableJobs.map(job => (
                <div key={job.id} className="glass-panel p-6 lg:px-8 lg:py-6 rounded-2xl flex flex-col lg:grid lg:grid-cols-12 gap-4 lg:gap-6 items-start lg:items-center hover:-translate-y-1 hover:border-[#D4AF37]/50 hover:shadow-[0_10px_30px_rgba(212,175,55,0.15)] transition-all duration-300 group relative overflow-hidden cursor-default">
                  {/* Hover Glow Sweep */}
...
                  
                  {/* Title & Role */}
                  <div className="col-span-3 w-full relative z-10">
                    <div className="font-bold text-gray-900 dark:text-white text-xl group-hover:text-[#D4AF37] transition-colors">{job.title}</div>
                    <div className="text-xs text-gray-500 uppercase tracking-widest font-black mt-1 flex items-center gap-1"><Briefcase size={12} className="text-[#D4AF37]"/> {job.role}</div>
                  </div>
                  
                  {/* Description */}
                  <div className="col-span-4 w-full relative z-10 text-sm text-gray-600 dark:text-gray-400 font-medium leading-relaxed">
                    {job.desc}
                  </div>
                  
                  {/* Location & Date */}
                  <div className="col-span-2 w-full relative z-10 space-y-1.5 flex flex-row lg:flex-col items-center lg:items-start justify-between lg:justify-start">
                    <div className="font-bold text-gray-700 dark:text-gray-300 flex items-center gap-2 text-sm"><MapPin size={14} className="text-[#D4AF37]"/> {job.location}</div>
                    <div className="text-xs font-medium text-gray-500 flex items-center gap-2"><Calendar size={12} className="text-[#D4AF37]"/> {job.date}</div>
                  </div>
                  
                  {/* Package */}
                  <div className="col-span-2 w-full relative z-10 flex lg:block justify-between items-center mt-2 lg:mt-0">
                    <span className="lg:hidden text-xs font-bold text-gray-500 uppercase tracking-wider">Package:</span>
                    <span className="inline-flex px-3 py-1 bg-green-500/10 text-green-600 dark:text-green-400 font-black rounded-lg text-sm border border-green-500/20">
                      {job.package}
                    </span>
                  </div>
                  
                  {/* Action */}
                  <div className="col-span-1 w-full lg:w-auto relative z-10 mt-4 lg:mt-0 flex justify-end">
                    <button onClick={() => navigateTo('contact')} className="w-full lg:w-auto px-6 py-3 lg:py-2.5 bg-transparent border border-[#D4AF37] text-[#D4AF37] hover:bg-[#D4AF37] hover:text-black font-bold rounded-xl transition-all shadow-sm whitespace-nowrap group-hover:shadow-[0_5px_15px_rgba(212,175,55,0.4)]">
                      Apply
                    </button>
                  </div>
                </div>
...
                <Search size={32} />
             </div>
            <h2 className="text-3xl md:text-5xl font-extrabold mb-6 text-gray-900 dark:text-white">Consulting <span className="text-[#D4AF37]">FAQ's</span></h2>
            <p className="text-lg text-gray-600 dark:text-gray-400">Everything you need to know about our recruitment integration process.</p>
          </div>
        </Reveal>
...
          {faqs.map((faq, i) => (
            <Reveal key={i} delay={i * 100}>
              <div className="glass-panel rounded-2xl overflow-hidden hover:border-[#D4AF37]/50 transition-colors">
                <button 
                  onClick={() => setActiveFaq(activeFaq === i ? null : i)}
                  className="w-full px-8 py-5 text-left flex justify-between items-center font-bold text-lg focus:outline-none text-gray-900 dark:text-white"
                >
                  {faq.q}
                  <ChevronDown className={`transform transition-transform duration-300 ${activeFaq === i ? 'rotate-180 text-[#D4AF37]' : 'text-gray-400'}`} />
                </button>
                <div className={`px-8 overflow-hidden transition-all duration-300 ease-in-out ${activeFaq === i ? 'max-h-40 pb-6 opacity-100' : 'max-h-0 opacity-0'}`}>
...
        { name: 'Job Opportunities', id: 'jobs-consulting', icon: <Target size={16} /> },
        { name: "Air Ticketing & Visa's", id: 'career-guide', icon: <Plane size={16} /> },
...
        {currentPage === 'jobs-consulting' && <JobsConsulting navigateTo={navigateTo} />}
        {currentPage === 'career-guide' && <AirTicketing navigateTo={navigateTo} isDarkMode={isDarkMode} />}
